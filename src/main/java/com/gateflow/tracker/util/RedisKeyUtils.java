package com.gateflow.tracker.util;

import java.nio.charset.StandardCharsets;

/**
 * Redis Key 工具类
 * 提供 Redis Cluster 槽位计算和分片 key 生成
 */
public final class RedisKeyUtils {
    ...
    /**
     * 计算 Redis Cluster 槽位
     * Redis Cluster 使用 CRC16(key) % 16384 计算槽位
     */
    public static int calculateSlot(String key) {
        String slotKey = extractSlotTag(key);
        byte[] bytes = slotKey.getBytes(StandardCharsets.UTF_8);
        int crc = 0x0000;
        for (byte b : bytes) {
            crc = (crc >>> 8) ^ CRC16_TABLE[(crc ^ (b & 0xFF)) & 0xFF];
        }
        return crc & 0x7FFF;  // 取低14位，即 % 16384
    }

    /**
     * 提取用于计算槽位的标签
     * Redis Cluster 使用 {tag} 之间的内容来计算槽位
     */
    public static String extractSlotTag(String key) {
    ...
    /**
     * 生成分片 Key（用于 Redis Cluster）
     * 使用 eventId 前8位作为 tag，保证同一 eventId 总是路由到同一槽位
     */
